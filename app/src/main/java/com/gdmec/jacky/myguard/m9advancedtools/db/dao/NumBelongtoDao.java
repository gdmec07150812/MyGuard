package com.gdmec.jacky.myguard.m9advancedtools.db.dao;
...
public class NumBelongtoDao {
    ...
    public static String getLocation(String phonenumber) {
        ...
        SQLiteDatabase db = SQLiteDatabase.openDatabase("/data/data/com.gdmec.jacky.myguard/files/address.db", null, SQLiteDatabase.OPEN_READONLY);
            ...
            switch (phonenumber.length()) {
                ...
                default:
                    if (location.length() >= 9 && location.startsWith("0")) {
                        ...
                        cursor.close();
                        cursor = db.rawQuery("select location from data2 where area=?", new
                                String[]{location.substring(1, 4)});
                        if (cursor.moveToNext()) {
                            String str = cursor.getString(0);
                            address = str.substring(0, str.length() - 2);
                        }
